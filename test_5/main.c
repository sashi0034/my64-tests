...
int main(void)
{
	...
	static char fired_text[FIRED_COUNT_MAX] = "";
	int fired_count = FIRED_COUNT_MAX;
	...
	while (1)
	{
		...
		snprintf(s_sb_128, sizeof(s_sb_128), "Z - Start / stop background music [%s]", music ? "ON" : "OFF");
		graphics_draw_text(disp, 50, 80, s_sb_128);
		...
		graphics_draw_text(disp, 50, 160, fired_text);
		display_show(disp);

		controller_scan();
		struct controller_data ckeys = get_keys_down();

		if (fired_count >= FIRED_COUNT_MAX - 1)
		{
			for (int i = 0; i < FIRED_COUNT_MAX; ++i)
				fired_text[i] = '\0';
			fired_count = 0;
		}
